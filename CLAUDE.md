...
- [x] Phase 6 — GraphRAG (optional): src/sourcerer/graphrag/ (extraction→graph/communities→summaries→store as JSON under GRAPHRAG_ROOT; local + global map-reduce search defined in graphrag/search.py, but ONLY global_search is wired into /query — local_search is implemented/exported yet not exposed on the endpoint. route_override graph-local/graph-api BOTH run global search; "local/api" picks where the reduce step runs, not the search mode). Indexing uses the LOCAL model (GRAPHRAG_EXTRACTION_MODEL) and is gated behind a confirm prompt (scripts/graphrag_index.py — EXPENSIVE). Router sends overview questions → graph global via /query when GRAPHRAG_ENABLED. scripts/graphrag_eval.py compares vs hybrid (quality + cost) on eval/overview_eval_set.jsonl. networkx is the [graphrag] extra. NOTE: index has NOT been built yet (needs the expensive run).
